...
export class Parser<T> {
  ...
  map<R>(callback: (result: T) => R): Parser<R> {
    return Parser.from((state) => {
      ...
      return ResultState.update(nextState, callback(nextState.result), 0)
    })
  }
  ...
  static from<T>(
    func: (state: ValidState<any>) => IntermediateState<T>
  ): Parser<T> {
    return new Parser<T>(func)
  }

  static of<T>(result: T): Parser<T> {
    return Parser.from((state) => ResultState.update(state, result, 0))
  }
